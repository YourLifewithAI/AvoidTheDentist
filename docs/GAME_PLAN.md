...
**Patient app (September 2026): built and published.** A phone-first, no-account version for patients, opened from a QR code: [yourlifewithai.github.io/AvoidTheDentist](https://yourlifewithai.github.io/AvoidTheDentist/). The poster is in `docs/qr/`.
- **Privacy:** a strict content security policy blocks all network requests, and nothing is stored.
- **Flow:** build a life from 13 habit questions, or pick one of 17 stories; watch it in the house; then the Life Story with the Acid Clock, garden, "change one thing" (averaged over 40 same-luck pairs), 100 lives and tool-shed picks.
- **Product links** live in `web/app/links.js`, empty until the clinical run-through.
